...
class DroneBlocksMissionClient(Node):
    ...
    def run_box_mission(self):
        ...
        if not self.execute_command('takeoff', parameter=3.0):
            return False

        # 3. Wait for takeoff to complete (auto takeoff doesn't block)
        self.get_logger().info('Waiting 10 seconds for auto takeoff to complete...')
        time.sleep(10.0)

        # 4. Now switch to offboard mode for precision flight
        ...
        # 5. Land
        if not self.execute_command('land'):
            return False
        ...
        # 8. Stop offboard heartbeat to return to normal state
        if not self.execute_command('stop_offboard_heartbeat'):
            return False
