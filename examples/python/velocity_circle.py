#!/usr/bin/env python3
"""
Velocity-based circle flight (1m radius).

Uses set_velocity_body with constant forward speed + yaw rate to trace a circle.
The relationship is: radius = forward_speed / yaw_rate_rad

With forward_speed=0.3 m/s and radius=1.0m:
  yaw_rate = 0.3 / 1.0 = 0.3 rad/s ≈ 17.19 deg/s
  duration = 2*pi*1.0 / 0.3 ≈ 20.9s

Usage:
    python3 velocity_circle.py
"""
...
CIRCLE_DURATION = (2.0 * math.pi * RADIUS) / FORWARD_SPEED  # ~20.9s


class VelocityCircle(Node):
    def __init__(self):
        super().__init__('velocity_circle')
        ...
        self.get_logger().info(
            f"Velocity circle: radius={RADIUS}m, speed={FORWARD_SPEED}m/s, "
            f"yaw_rate={YAW_RATE:.1f}deg/s, duration={CIRCLE_DURATION:.1f}s"
        )
    ...
    def execute(self):
        try:
            self.get_logger().info("=== Velocity Circle (1m radius) ===")
            ...
            # Fly circle: forward + yaw right
            self.get_logger().info(f"Flying circle for {CIRCLE_DURATION:.1f}s...")
            self.send_velocity(FORWARD_SPEED, 0.0, 0.0, YAW_RATE, CIRCLE_DURATION)
